...
def gspo_default_reward(completions, **kwargs) -> list[float]:
    """Sequence-level default reward for GSPO.

    This reward is designed to be robust when gold solutions are missing:
    - Enforce response structure (<think>/<answer>)
    - Encourage substantial answers (neither empty nor overly short)
    - Penalize repetitive/degenerate outputs
    - Penalize refusal-style non-answers
    """
    ...
    for content in contents:
        ...
        if match is not None:
            ...
            # Keep a small path to positive reward even without strict tags.
        ...
        # Light repetition penalty on 3-grams in the answer region.
...
def get_reward_funcs(script_args) -> list[Callable]:
    REWARD_FUNCS_REGISTRY = {
        "gspo_default": gspo_default_reward,
