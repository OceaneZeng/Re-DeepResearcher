...
from rl.configs import GSPOConfig, GSPOScriptArguments
from rl.rewards import get_reward_funcs
...
try:
    from trl import GSPOTrainer
except ImportError as exc:
    GSPOTrainer = None
    GSPO_IMPORT_ERROR = exc
else:
    GSPO_IMPORT_ERROR = None
...
def main(script_args, training_args, model_args):
    if GSPOTrainer is None:
        raise ImportError(
            "GSPOTrainer is not available in your installed TRL version. "
            "Please upgrade TRL to a version that includes GSPO, then rerun. "
            f"Original import error: {GSPO_IMPORT_ERROR}"
        )
    ...
    try:
        ...
        if getattr(script_args, "eos_token", None) and len(tokenizer) > getattr(
            model.get_input_embeddings(), "num_embeddings", len(tokenizer)
        ):
